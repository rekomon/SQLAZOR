...
namespace SQLAZOR.Services
{
    public sealed class CrudGeneratorService : ICrudGeneratorService    
    {
        ...
        private static GeneratedFile GenerateController(TableInfo table, string rootNamespace)
        {
            ...
            var routeTemplate = BuildPkPathSegment(table);
        ...
        {
            var pkParams = BuildPkMethodParams(table);
            var pathSegment = BuildPkPathSegment(table); // e.g. "{id}" or "{orderId}/{lineNumber}" - valid as a C# interpolation body too
        ...
        private static GeneratedFile GenerateListPage(TableInfo table, string rootNamespace, PageStyle pageStyle)
        {
            ...
            var editUrlBody = "/" + pluralRoute + "/edit/" + string.Join("/", GetPkArgs(table).Select(a => "{item." + a.Column.PropertyName + "}"));
      ...
      string.Join(" AND ", GetPkArgs(table).Select(a => $"[{a.Column.ColumnName}] = @{a.ArgName}"));

        private static string BuildPkParamsObject(TableInfo table) =>
            "new { " + string.Join(", ", GetPkArgs(table).Select(a => a.ArgName)) + " }";
        private static string BuildPkArgsFromItem(TableInfo table, string itemVar) =>
       string.Join(", ", GetPkArgs(table).Select(a => $"{itemVar}.{a.Column.PropertyName}"));
        ...
        private static string BuildPkPathSegment(TableInfo table) =>
            string.Join("/", GetPkArgs(table).Select(a => "{" + a.ArgName + "}"));



        private static List<(string ArgName, ColumnInfo Column)> GetPkArgs(TableInfo table)
        {
        ...
        private static string BuildPkMethodParams(TableInfo table) =>
            string.Join(", ", GetPkArgs(table).Select(a => $"{a.Column.ClrType.TrimEnd('?')} {a.ArgName}"));
